      ...
      <div className="pt-12 px-6 pb-6">
        ...
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 before:to-transparent">
          <div className="relative flex items-center justify-between group is-active">
            <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white bg-green-500 text-white shadow shrink-0"><FaCheckCircle /></div>
            <div className="w-[calc(100%-4rem)] bg-white p-4 rounded-xl shadow-sm border"><h3 className="font-bold text-gray-800">Order Confirmed</h3><p className="text-xs text-gray-500">We have received your order.</p></div>
          </div>
          <div className="relative flex items-center justify-between group is-active">
            <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white text-white shadow shrink-0 ${order.status === 'processing' || order.status === 'shipped' || order.status === 'delivered' ? 'bg-green-500' : 'bg-gray-300'}`}><FaBoxOpen /></div>
            <div className="w-[calc(100%-4rem)] bg-white p-4 rounded-xl shadow-sm border"><h3 className="font-bold text-gray-800">Packing Order</h3><p className="text-xs text-gray-500">Items are being packed securely.</p></div>
          </div>
          <div className="relative flex items-center justify-between group is-active">
            <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white text-white shadow shrink-0 ${order.status === 'shipped' || order.status === 'delivered' ? 'bg-green-500' : 'bg-gray-300'}`}><FaMotorcycle /></div>
            <div className="w-[calc(100%-4rem)] bg-white p-4 rounded-xl shadow-sm border"><h3 className="font-bold text-gray-800">Out for Delivery</h3><p className="text-xs text-gray-500">Raju Bhaiya is on the way.</p></div>
          </div>
        </div>
